...
// Схема валидации для авторизации устройства
...
/**
 * POST /api/cabinets/authorize-device
 ...
 *   public_key: string (Ed25519 публичный ключ в hex)
 *   user_agent?: string
 ...
 * }
 */
router.post('/authorize-device', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const pool = getPool();
    
    // Валидация входных данных
    const { session_token, device_fingerprint, public_key, user_agent, screen_resolution, timezone } = authorizeDeviceSchema.parse(req.body);
    
    // Проверка session_token и срока действия
    ...
    if (sessionResult.rows.length === 0) {
      throw createError('Session token not found or expired', 404);
    }
    
    const session = sessionResult.rows[0];
    
    // Проверка: сессия должна быть подтверждена
    if (!session.confirmed_at) {
      throw createError('Session token has not been confirmed yet', 400);
    }
    
    const cabinetId = session.cabinet_id;
    
    // Форматирование публичного ключа
    const formattedPublicKey = formatPublicKey(public_key);
    
    // Проверка: устройство уже авторизовано для этого кабинета?
    const existingDevice = await pool.query(
      `SELECT id FROM authorized_devices
       WHERE cabinet_id = $1 AND device_fingerprint = $2`,
      [cabinetId, device_fingerprint]
    );
    
    if (existingDevice.rows.length > 0) {
      // Обновляем last_used_at и public_key (на случай, если ключ изменился)
      await pool.query(
        `UPDATE authorized_devices
         SET public_key = $1, last_used_at = CURRENT_TIMESTAMP
         WHERE id = $2`,
        [formattedPublicKey, existingDevice.rows[0].id]
      );
      
      logger.info(`Device re-authorized: cabinet_id=${cabinetId}, device_fingerprint=${device_fingerprint.substring(0, 16)}...`);
    ...
    // Создание записи в authorized_devices
    const deviceResult = await pool.query(
      `INSERT INTO authorized_devices 
       (cabinet_id, device_fingerprint, public_key, authorized_at, last_used_at)
       VALUES ($1, $2, $3, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
       RETURNING id`,
      [cabinetId, device_fingerprint, formattedPublicKey]
    );
    
    const deviceId = deviceResult.rows[0].id;
    
    logger.info(`Device authorized: cabinet_id=${cabinetId}, device_id=${deviceId}, device_fingerprint=${device_fingerprint.substring(0, 16)}...`);
